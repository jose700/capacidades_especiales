...
class AgregarInscripcionMateria extends StatefulWidget {
  final int? idinscripcion;
  final int idestudiante;
  final String usuario;
  final int idmateria;
  final Sesion sesion;
  const AgregarInscripcionMateria({
    Key? key,
    this.idinscripcion,
  ...
  @override
  _AgregarInscripcionMateriaState createState() =>
      _AgregarInscripcionMateriaState();
}

class _AgregarInscripcionMateriaState extends State<AgregarInscripcionMateria> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Agregar Inscripción Materia'),
        elevation: 0.0,
      ),
                ...
                ElevatedButton(
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      // Crear la nueva inscripción con los datos ingresados
                      InscripcionMateria nuevaInscripcion = InscripcionMateria(
                        idinscripcion: widget.idinscripcion,
                      ...
                      try {
                        await servicioInscripcionMateria.crearInscripcion(
                            nuevaInscripcion, widget.sesion.token);
                        Dialogs.showSnackbar(context,
                            'Se ha guardado la inscripción correctamente');
